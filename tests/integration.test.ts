...
describe("CLI Integration Tests", () => {
  ...
  describe("CLI Basic Functionality", () => {
    ...
    test("should perform dry run rename", async () => {
      const { stdout } = await execAsync(
        `bunx ${cliPath} --old-name old-package --new-name new-package --dry`,
        { cwd: testDir }
      );
    ...
    test("should update package.json when requested", async () => {
      const { stdout } = await execAsync(
        `bunx ${cliPath} --old-name old-package --new-name new-package --update-package --dry`,
        { cwd: testDir }
      );
    ...
    test("should handle case sensitive option", async () => {
      ...
      const { stdout } = await execAsync(
        `bunx ${cliPath} --old-name OldPackage --new-name NewPackage --case-sensitive --dry`,
        { cwd: testDir }
      );
    ...
    test("should handle multi-map renames", async () => {
      ...
      const { stdout } = await execAsync(
        `bunx ${cliPath} --old-name pkg1 --new-name newpkg1 --multi-map '${multiMap}' --dry`,
        { cwd: testDir }
      );
  ...
  describe("CLI Error Handling", () => {
    test("should handle invalid old name", async () => {
      try {
        await execAsync(
          `bunx ${cliPath} --old-name '' --new-name new-package --dry`,
          { cwd: testDir }
        );
        // If we reach here, the test should fail
        expect(true).toBe(false);
    ...
    test("should handle invalid new name", async () => {
      try {
        await execAsync(
          `bunx ${cliPath} --old-name old-package --new-name '' --dry`,
          { cwd: testDir }
        );
        // If we reach here, the test should fail
        expect(true).toBe(false);
    ...
    test("should handle same old and new names", async () => {
      try {
        await execAsync(
          `bunx ${cliPath} --old-name same --new-name same --dry`,
          { cwd: testDir }
        );
        // If we reach here, the test should fail
        expect(true).toBe(false);
    ...
    test("should handle non-existent directory", async () => {
      ...
      try {
        await execAsync(
          `bunx ${cliPath} --old-name old --new-name new --dry`,
          { cwd: nonExistentDir }
        );
        // If we reach here, the test should fail
        expect(true).toBe(false);
  ...
  describe("CLI Configuration Integration", () => {
    test("should load configuration from .saferenamerc.json", async () => {
      ...
      const { stdout } = await execAsync(
        `bunx ${cliPath} --old-name old-package --new-name new-package`,
        { cwd: testDir }
      );
    ...
    test("should merge CLI args with config file", async () => {
      ...
      const { stdout } = await execAsync(
        `bunx ${cliPath} --old-name old-package --new-name new-package --update-package`,
        { cwd: testDir }
      );
  ...
  describe("CLI Performance Tests", () => {
    test("should handle large number of files", async () => {
      ...
      const { stdout } = await execAsync(
        `bunx ${cliPath} --old-name old-package-1 --new-name new-package-1 --dry`,
        { cwd: testDir }
      );
