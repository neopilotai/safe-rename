...
import { intro, outro, text, select, confirm, multiselect, spinner, note, cancel, isCancel } from "@clack/prompts";
import fs from "fs-extra";
import path from "path";

// Beautiful CLI visualization functions
function showBox(title: string, content: string, width: number = 60) {
...
function showScriptBox(script: string, explanation: string) {
...
function showConfirmBox(question: string) {
...
async function analyzeRepository() {
  showScriptBox(
    "safe-rename --analyze",
    "Scans repository files to identify frequently used tokens.\n" +
...
async function interactiveMode() {
  showScriptBox(
    "safe-rename --interactive",
    "Launches interactive CLI wizard for safe rename operations.\n" +
...
async function handleSingleRename(config: any) {
  const oldName = await text({
    message: "Enter the old name to rename:",
    placeholder: "old-package",
    validate: (value) => {
      if (!value || !value.trim()) return "Old name is required";
  ...
  const newName = await text({
    ...
    validate: (value) => {
      if (!value || !value.trim()) return "New name is required";
      if (value === oldName) return "New name must be different from old name";
      if (value.length > 255) return "Name must be less than 255 characters";
      if (!/^[a-zA-Z0-9\-_/@]+$/.test(value)) return "Name contains invalid characters";
    }
  });
  ...
  const concurrency = await text({
    ...
    validate: (value) => {
      if (!value) return "Concurrency is required";
...
async function handleMultiRename(config: any) {
  ...
  const multiMapText = await text({
    ...
    validate: (value) => {
      if (!value || !value.trim()) return "Multi-map is required";
      try {
...
async function main() {
  try {
    ...
    if (!args.quiet) {
      showScriptBox(
        "safe-rename --old-name <old> --new-name <new>",
        `Enterprise-grade repository refactoring tool.\n\n` +
