...
export function validateRenameOptions(
  oldName: string,
  newName: string,
): ValidationResult {
...
export function validateMultiMap(
...
export class ValidationError extends SafeRenameError {
  ...
  constructor(validationErrors: ValidationIssue[]) {
    const message =
      formatValidationErrors(validationErrors) || "Validation failed";
