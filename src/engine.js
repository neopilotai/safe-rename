...
export async function runSafeRename(ctx) {
    const { cwd, args } = ctx;
    // Load configuration
    ...
    console.log(`🔄 Starting safe rename: ${options.oldName} → ${options.newName}`);
    if (options.dry)
        console.log("🔍 DRY RUN MODE - no files will be modified");
    const files = await findFilesToProcess(cwd, options);
    console.log(`📁 Found ${files.length} files to process`);
    const limit = pLimit(options.concurrency || 10);
    const results = await Promise.all(files.map((file) => limit(() => processFile(file, options))));
    ...
    console.log(`✅ Summary: ${summary.processed} processed, ${summary.modified} modified, ${summary.errors} errors`);
...
async function findFilesToProcess(root, options) {
...
function processTypeScriptFile(content, options) {
    let newContent = content;
    const renameMap = options.multiMap || { [options.oldName]: options.newName };
    for (const [oldName, newName] of Object.entries(renameMap)) {
        const flags = options.caseSensitive ? "g" : "gi";
        newContent = newContent.replace(new RegExp(`(import\\s+.*?\\s+from\\s+['"][^'"]*)${oldName}([^'"]*['"])`, flags), `$1${newName}$2`);
        newContent = newContent.replace(new RegExp(`(export\\s+.*?\\s+from\\s+['"][^'"]*)${oldName}([^'"]*['"])`, flags), `$1${newName}$2`);
        newContent = newContent.replace(new RegExp(`(require\\s*\\(\\s*['"][^'"]*)${oldName}([^'"]*['"])`, flags), `$1${newName}$2`);
        newContent = newContent.replace(new RegExp(`(@${oldName})`, flags), `@${newName}`);
        newContent = newContent.replace(new RegExp(`\\b${oldName}\\b`, flags), newName);
    }
    return newContent;
}
function processJsonFile(content, options) {
    try {
        ...
        console.warn(`⚠️  Failed to parse JSON file, treating as plain text: ${error.message}`);
        return content;
    }
}
function processYamlFile(content, options) {
    try {
        ...
        console.warn(`⚠️  Failed to parse YAML file, treating as plain text: ${error.message}`);
        return content;
    }
}
function processEnvFile(content, options) {
    let newContent = content;
    const renameMap = options.multiMap || { [options.oldName]: options.newName };
    for (const [oldName, newName] of Object.entries(renameMap)) {
        const flags = options.caseSensitive ? "g" : "gi";
        newContent = newContent.replace(new RegExp(`(${oldName.toUpperCase()})`, flags), newName.toUpperCase());
        newContent = newContent.replace(new RegExp(`=${oldName}`, flags), `=${newName}`);
    }
    return newContent;
}
function processDockerFile(content, options) {
    let newContent = content;
    const renameMap = options.multiMap || { [options.oldName]: options.newName };
    for (const [oldName, newName] of Object.entries(renameMap)) {
        const flags = options.caseSensitive ? "g" : "gi";
        const fromRegex = new RegExp(`(FROM|image:)\\s*([^\\s]*${oldName}[^\\s]*)`, flags);
        newContent = newContent.replace(fromRegex, (match, prefix, imagePath) => {
            const newImagePath = imagePath.replace(new RegExp(oldName, flags), newName);
            return `${prefix} ${newImagePath}`;
        });
        newContent = newContent.replace(new RegExp(`(ENV|ARG)\\s+(${oldName.toUpperCase()})`, flags), `$1 ${newName.toUpperCase()}`);
    }
    return newContent;
}
function processMarkdownFile(content, options) {
    let newContent = content;
    const renameMap = options.multiMap || { [options.oldName]: options.newName };
    for (const [oldName, newName] of Object.entries(renameMap)) {
        const flags = options.caseSensitive ? "g" : "gi";
        const codeBlockRegex = new RegExp('(\\\\`\\\\`\\\\[\\\\s\\\\S]*?\\\\`\\\\`\\\\`|\\\\`[^\\\\`]*\\\\`)', flags);
        newContent = newContent.replace(codeBlockRegex, (match) => match.replace(new RegExp(oldName, flags), newName));
        const linkRegex = new RegExp("\\[([^\\]]*)\\]\\(([^)]*" + oldName + "[^)]*)\\)", flags);
...
async function updatePackageJson(root, options) {
    ...
    if (await fs.pathExists(packageJsonPath)) {
        ...
        ["dependencies", "devDependencies", "peerDependencies"].forEach((depType) => {
            if (packageJson[depType]) {
                ...
                for (const [oldName, newName] of Object.entries(renameMap)) {
                    const oldDepKey = Object.keys(packageJson[depType]).find((key) => key.includes(oldName));
                    if (oldDepKey) {
                        const newDepKey = oldDepKey.replace(new RegExp(oldName, options.caseSensitive ? "g" : "gi"), newName);
...
async function updateGitRemote(root, options) {
    ...
    try {
        ...
        for (const [oldName, newName] of Object.entries(renameMap)) {
            const remoteRegex = new RegExp(`(origin\\s +.*? ${oldName}.*?) \\s +\\(fetch\\)`, "gi");
            const match = stdout.match(remoteRegex);
            if (match) {
                const oldUrl = match[0].replace(" (fetch)", "").replace("origin\t", "");
                const newUrl = oldUrl.replace(new RegExp(oldName, options.caseSensitive ? "g" : "gi"), newName);
...
export { processTypeScriptFile, processJsonFile, processYamlFile, processEnvFile, processDockerFile, processMarkdownFile, updatePackageJson, updateGitRemote, };
